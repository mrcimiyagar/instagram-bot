    ...
    sw.Session.findOne({where: {token: req.body.token}}).then(function (session) {
        ...
        sw.InstaAccount.findOne({where: { [op.and]: [{userId: session.userId}, {instaAccountId: req.body.instaAccountId}]}}).then(function (instaAcc) {
            ...
            sw.Comment.findOne({where: { [op.and]: [ {username: req.body.username}, {instaAccountId: instaAcc.instaAccountId}]}}).then(async function (like) {
                ...
                res.send({status: 'success', commentTarget: result, message: "Comment target created successfully."});
            });
        });
    });
});
...
router.post('/remove_comment_target', function (req, res) {
    sw.Session.findOne({where: {token: req.body.token}}).then(function (session) {
        ...
        sw.InstaAccount.findOne({where: { [op.and]: [{userId: session.userId}, {instaAccountId: req.body.instaAccountId}]}}).then(function (instaAcc) {
            ...
            sw.Comment.findOne({where: { [op.and] : [ {commentId: req.body.commentId}, {instaAccountId: instaAcc.instaAccountId}]}}).then(async function (comment) {
                if (comment === null) {
                ...
                ipb.runInstaAgent(instaAcc.instaAccountId, instaAcc.username, instaAcc.password);
                res.send({status: 'success', commentTarget: comment, message: "Comment target removed successfully."});
            });
        });
    });
});
    ...
    sw.Session.findOne({where: {token: req.body.token}}).then(function (session) {
        ...
        sw.InstaAccount.findOne({where: { [op.and]: [{userId: session.userId}, {instaAccountId: req.body.instaAccountId}]}}).then(function (instaAcc) {
            ...
            sw.Comment.findOne({where: {instaAccountId: instaAcc.instaAccountId}}).then(async function (comments) {
                res.send({status: 'success', commentTargets: comments, message: "Comment targets fetched successfully."});
            });
        });
    });
});

...
class ModifiedInstagramAPI(InstagramAPI):
    def SendRequest(self, endpoint, post=None, login=False):
        ...
        try:
            ...
            print("SendRequest: exception: "+str(e))
            return None
...
class InstaBot:
    ...
    def set_follow_targets(self, follow_targets):
        self.target_user_list = follow_targets
    ...
    def send_request(self, request, *args, **kwargs):
        ...
        self.wait()
        ret = None
        print("send_request; Reguest=" + request.__name__ + "| args=" + str(args) + " | kwargs=" + str(kwargs))
        try:
            response = request(*args, **kwargs)
            if response.status_code == 200:
                print("send_request; http 200")
                ret = json.loads(response.text)
            elif response.status_code == 404:
                print("send_request; http 404")
                ret = 404
            else:
                print("send_request; Response:  " + str(response))
                print("send_request; Response text:  " + str(response.text))
                if response.status_code in [400, 429]:
                    try:
                        if json.loads(response.text)['spam']:
                            print("send_request; Spam Detected")
                            print("Spam Detected")
                            print("Waking up at",
                        ...
                        print("send_request; Possible Rate Limiting Detected")
                        sleep(5 * 60)

        except Exception as e:
            print("send_request; Exception: " + str(e))
    ...
    def unfollow_user(self, user_id, followed_queue):
        ...
        if ret is None:
            print("unfollow_users: unfollow fail")
    ...
    def target_finder(self):
        ...
        def get_followback_confidence(user_info):
            ...
            try:
                ...
                print("get_followback_confidence, model not fitted")
        ...
        try:
            for username in self.target_user_list:
                print("find_targets: populate target user info for " + username)
                try:
                    target_user_infos[username] = self.send_request(self.api.searchUsername, username)
                except Exception as e:
                    print("find_targets, create_user_iterator, username=" + username + "; Exception: " + str(e))
        except Exception as e:
            print("find_targets, create_user_iterators; Exception: " + str(e))
            ...
            while self.below_explore_limit():
                try:
                    ...
                    if np.random.rand() < epsilon:
                        print("find_targets: mark target for exploration")
                        explore = True
                    ...
                    if len(self.target_user_list) == 0 or np.random.rand() < 0.5:
                        ...
                        if len(tag_iterators[tag]) == 0:
                            try:
                                print("find_targets: refresh list for " + tag)
                    ...
                    print(
                        "find_targets: selected tag=" + tag + " (" + ("hashtag)" if tag in self.tag_list else "user)"))
                    ...
                    if self.valid_target(user_id):
                        ...
                        if user_followers is not None:
                            ...
                            print("find_targets: enque target for " + tag + " (confidence=" + str(
                                followback_confidence) + ")")
                            self.targets_queue.put(user_info, followback_confidence)
                            self.hour_explores.put(user_id)
                except Exception as e:
                    print("find_targets; Exception: " + str(e))
    ...
    def like_follow_unfollow(self):
        ...
        def target_users():
            ...
            def target_user(user_id):
                ...
                if items is None:
                    print("like_follow_unfollow: target_users: target_user: user feed 404")
                    return
            ...
            while self.below_follow_limit() and self.below_like_limit():
                ...
                if user_info is not None:
                    ...
                    if self.valid_target(user_id):
                        ...
                        print("like_follow_unfollow: target_users: target_user " + str(user_id))
